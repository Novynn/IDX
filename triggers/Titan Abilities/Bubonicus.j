...
library Bubonicus requires GameTimer, GT, Table, Transport {
    ...
    public struct Bubonicus {
        ...
        private method onDropEffect() {
            ...
            if (IsTerrainShallowWater(x, y) || IsTerrainDeepWater(x, y)) {
                this.error("|cffff0000You may not spawn Corpse Wards on water.|r");
            }
            else if (IsPointBlighted(x, y)) {
                this.error("|cffff0000You may not spawn Corpse Wards on blighted ground.|r");
            }
            else {
        ...
        public static method onWardDeath(unit u) {
            SetBlight(GetOwningPlayer(u), GetUnitX(u), GetUnitY(u), 48.0, false);
            RemoveUnit(u);
            u = null;
        }
