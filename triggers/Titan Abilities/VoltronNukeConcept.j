...
// TVAQ
library VoltronNuke requires GameTimer, GT, xebasic, xepreload, xecollider, GenericTitanTargets {
    private struct VoltronNukeBall extends xecollider {
        private VoltronNuke object = 0;
        public method setNukeObject(VoltronNuke object){
            this.object = object;
        }
    ...
    private struct VoltronNuke {
        private static constant integer ABILITY_ID = 'TVAQ';
        ...
        private VoltronNukeBall ball[3];
        ...
        private method generateBalls(real centerAngle, real delta, real collision) {
            ...
            for (-1 <= i <= 1) {
                this.ball[i+1] = VoltronNukeBall.create(castX, castY, (centerAngle + (delta * i)) * bj_DEGTORAD);
        ...
        method ballEnd(VoltronNukeBall b) {
            integer i = 0;
            integer count = 0;
            for (0 <= i < 3) {
                if (b == this.ball[i]) {
        ...
        private static method onCast(){
            ...
            VoltronNuke.begin(caster, x, y, level);
        }
