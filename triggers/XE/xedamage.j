//TESH.scrollpos=16
//TESH.alwaysfold=0
library xedamage initializer init requires xebasic, Damage
//************************************************************************
// xedamage 0.8
// --------
...
//===========================================================================================================
    ...
    struct xedamage[MAX_SPACE]
        ...
        public boolean   forceEffect = false
        ...
        static method getDamageTypeFactor takes unit u, attacktype a, damagetype d returns real
         ...
         local real fc = FACTOR_TEST_DAMAGE
            ...
            call UnitDamageTargetEx(dmger,u, fc ,false,false,a,d,null)
        ...
        method damageTargetForceValue takes unit source, unit target, real damage returns nothing
         ...
         local integer    tg=.CurrentDamageTag
                ...
                set .inUse = .inUse +1
                call UnitDamageTargetEx(source,target,  damage, true, .ranged, null, null, .wtype )
                set .inUse = .inUse -1
                set .CurrentDamageTag  = tg
                set .CurrentDamageType = dt
                set .CurrentAttackType = at
         ...
         local real hp
            ...
            loop
                ...
                if (f!=0.0) then
                    ...
                    if(usefx and (hp > GetWidgetLife(target)) and not this.forceEffect) then
                        call DestroyEffect(  AddSpecialEffectTarget(this.fxpath, target, this.fxattach) )
                    endif   
                endif
            endloop
        ...
        private static method damageAOE_Enum takes nothing returns boolean
         ...
         local real hp
             ...
             if(f!=0.0) then
                 ...
                 call UnitDamageTargetEx(.sourceAOE,target, f*this.AOEdamage, true, .ranged, .atype, .dtype, .wtype )
                 if(this.usefx and ((hp > GetWidgetLife(target) ) and not this.forceEffect )) then
                     call DestroyEffect(  AddSpecialEffectTarget(this.fxpath, target, this.fxattach) )
                 endif                                  
             endif
        ...
        private static method damageAOE_DestructablesEnum takes nothing returns boolean
         ...
         local real     dy=.AOEy-GetDestructableY(target)
             ...
             call UnitDamageTargetEx(.sourceAOE,target, this.AOEdamage, true, .ranged, .atype, .dtype, .wtype )
