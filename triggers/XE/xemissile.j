//TESH.scrollpos=265
//TESH.alwaysfold=0
library xemissile requires xefx, xebasic, GetLocationZEx
//****************************************************************
//*
//* xemissile 0.9
//* -------------
...
//****************************************************************
    ...
    private struct missile extends eventHandler
        ...
        public method operator targetUnit= takes unit u returns nothing
            ...
            set .tx=GetUnitX(u)
            set .ty=GetUnitY(u)
            set .tz=GetUnitFlyHeight(u)+XYGetZ(.tx,.ty)+.zoffset
        endmethod
            ...
            loop
                ...
                if .dead then
                    ...
                    if .tu!=null and GetUnitTypeId(.tu)!=0 then
                        ...
                        set .tx=GetUnitX(.tu)
                        set .ty=GetUnitY(.tu)
                        set .tz=GetUnitFlyHeight(.tu)+XYGetZ(.tx,.ty)+.zoffset
                    endif
                    if .update then
                        set .update=false
                        set dx=.tx-.mx
