...
towerRanges['o004'] = 900; // |cffff0000Mega Dart Tower|r
...
towerRanges['o001'] = 700; // |cffef4500Super Dart Tower|r
...
towerRanges['o000'] = 600; // |cffcf9633Enhanced Dart Tower|r
...
towerRanges['o01Z'] = 1000; // Mega Long Dart Tower
//! endtextmacro
...
library TowerRange requires GT, Table, GameTimer {
    ...
    private struct TowerRange {
        static Table towerRanges = 0;
        static constant integer ABILITY_ID = 'A0H9';
        static constant string MODEL_PATH = "Abilities\\Weapons\\SpiritOfVengeanceMissile\\SpiritOfVengeanceMissile.mdl";
        static constant real BUMP = 80.0;
        
        private static method upgradeBonus(player p, integer id) -> real {
        ...
        private static method onAct() {
            unit u = GetTriggerUnit();
            player p = GetOwningPlayer(u);
            integer id = GetUnitTypeId(u);
            real dist = towerRanges[id] + thistype.BUMP + thistype.upgradeBonus(p, id);
            string s = "<BLANK>";
            
            debug {
                BJDebugMsg("Base Range: " + R2S(towerRanges[id]) + "\n" +
                           "Bonus: " + R2S(thistype.upgradeBonus(p, id)) + "\n" +
                           "Total: " + R2S(dist - thistype.BUMP));
            }
