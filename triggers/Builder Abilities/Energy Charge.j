...
library EnergyCharge {
    private constant integer ENERGY_CHARGE_SPELLBOOK = 'A00R';
    ...
    private function act(){
        ...
        boolexpr b = Filter(function () -> boolean {
            return (IsUnitType(GetFilterUnit(), UNIT_TYPE_GROUND) == true) &&
                   (GetUnitAbilityLevel(GetFilterUnit(), ENERGY_CHARGE_SPELLBOOK) > 0 ||
        ...
        while(u != null){
            if ((GetUnitAbilityLevel(u, ENERGY_CHARGE_BUFF) > 0) &&
                (GetUnitAbilityLevel(u, ENERGY_CHARGE_SPELLBOOK) == 0)){
                UnitAddAbility(u, ENERGY_CHARGE_SPELLBOOK);
            }
            else if ((GetUnitAbilityLevel(u, ENERGY_CHARGE_BUFF) == 0) &&
                     (GetUnitAbilityLevel(u, ENERGY_CHARGE_SPELLBOOK) > 0)){
                UnitRemoveAbility(u,ENERGY_CHARGE_SPELLBOOK);
            }
    ...
    private function onInit(){
        ...
        for (0 <= i < bj_MAX_PLAYERS){
            SetPlayerAbilityAvailable(Player(i), ENERGY_CHARGE_SPELLBOOK, false);
        }
    }
}
